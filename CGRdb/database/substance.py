...
class Substance(Entity):
    ...
    def __init__(self, substance: Optional[
        Iterable[Tuple[MoleculeContainer, Optional[float]]]] = None, /,
                 name=None):
        """
        :param substance:
        """
        ...
        if substance is not None:
            if not isinstance(substance, abcIterable):
                raise ValueError(
                    "Iterable type consisted of tuples should be provided")
            substance = list(substance)
            for mol, frac in substance:
                if not isinstance(mol, MoleculeContainer):
                    raise ValueError(
                        'First element of tuple should be CGRtools.MoleculeContainer')
...
class SubstanceStructure(Entity):
    ...
    def __init__(self,
                 structure: Union['molecule.Molecule', 'molecule.NonOrganic'],
                 substance: Substance, /, *,
                 molar_fraction: Optional[float] = None,
                 mapping: Union[
                     Dict[int, int], List[Tuple[int, int]], None] = None):
        if isinstance(mapping, dict):
            if not all(isinstance(x, int) for x in mapping.items() for x in x):
                raise TypeError(
                    'Mapping is dict that contains following structure: '
                    '{int (atom of substance) : int (atom of NonOrganic or Molecule)}')
            mapping = list(mapping.items())
        elif isinstance(mapping, (tuple, list)):
            if not all(isinstance(x, (tuple, list)) and len(x) == 2 and all(
                    isinstance(x, int) for x in x)
                       for x in mapping):
                raise TypeError(
                    'For the tuple pairs of integers following constructions should be applied: '
                    '[(int (atom of substance), int (atom of NonOrganic or Molecule)),...]')
        elif mapping is not None:
            raise TypeError(
                'Mapping of Substance to NonOrganic or Molecule expected')
        ...
        if isinstance(structure, molecule.Molecule):
            ...
            raise TypeError(
                'At least one either Molecule or NonOrganic required')
        super().__init__(molar_fraction=molar_fraction, molecule=mol,
                         _mapping=mapping,
    ...
    def structure(self):
        if self.molecule:
            if self.mapping:
                return self.molecule.canonic_structure.structure.remap(
                    self.mapping, copy=True)
            return self.molecule.canonic_structure.structure
        else:
            if self.mapping:
                return self.non_organic.structure.remap(self.mapping,
                                                        copy=True)
