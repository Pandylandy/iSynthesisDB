...
class Molecule(db.Entity):
    ...
    name = Required(str)
    organic_molecule = Required(bool)
    structure = Set('Molecule_Structure')
    properties = Set('Molecule_Property')
    storage = Set('Storage')
    reactions = Set('MoleculeReaction')

    def __init__(self, structure, organic_molecule=True,
                 substance_form='No information', remaining=None,
                 shelf_life='01.01.1900', place='No information',
                 supplier='No information',
                 canonical_structure='No information'):
        if Molecule_Structure.exists(signature=bytes(structure)):
            raise ValueError('That molecule is already in the database >:-(')
        db.Entity.__init__(self, name=str(structure),
                           organic_molecule=organic_molecule)
        Molecule_Structure(molecule=self, signature=bytes(structure),
                           data=structure,
                           canonical_structure=canonical_structure)
        Molecule_Property(molecule=self, substance_form=substance_form,
    ...
    def __find_reactions_by_molecule(cls, molecule, product):
        ...
        if finding_mol:
            reactions = [i.reaction for i in finding_mol.reactions if
                         i.molecule == finding_mol and i.product == product]
        return reactions
...
class Molecule_Property(db.Entity):
    ...
    def __init__(self, molecule, substance_form, remaining, shelf_life):
        if Molecule_Property.exists(molecule=molecule,
                                    substance_form=substance_form,
                                    remaining=remaining,
                                    shelf_life=shelf_life):
            raise ValueError(
                'That property information is already in the database >:-(')
        db.Entity.__init__(self, molecule=molecule,
                           substance_form=substance_form, remaining=remaining,
                           shelf_life=shelf_life)

    # Определяем метод, который показывает заканчивающиеся в ближайшие 180 дней сроки годности молекул
    @classmethod
    def upcoming_180_days(cls):
        return [x.molecule for x in cls.select(
...
class Molecule_Structure(db.Entity):
    ...
    def __init__(self, molecule, data, signature, canonical_structure):
        data = dumps(data)
        db.Entity.__init__(self, molecule=molecule, signature=signature,
                           data=data, canonical_structure=canonical_structure)
...
class Storage(db.Entity):
    molecule = Required(Molecule)
    id = PrimaryKey(int,
                    auto=True)  # Identification number of the molecule storage rules
    storage_conditions = Optional(Json)
    place = Required(str)
    supplier = Optional(str)

    def __init__(self, molecule, place='No information',
                 supplier='No information'):
        db.Entity.__init__(self, molecule=molecule, place=place,
                           supplier=supplier)
...
class Reaction(db.Entity):
    ...
    def structure(self):
        ...
        return ReactionContainer(reactants=list_of_reactants,
